...
// mocked) so THIS dialog's placement/elevation/focus/surface-anchor contract holds:
// 240px, elevation-400, left-anchored collision-safe, focus return, Escape, and the
// side axis anchored to the inspector SURFACE edge (#499/#73), not the trigger.
...
function renderOpen(mode: "light" | "dark", onClose = vi.fn()) {
  ...
  act(() => {
    renderer = create(
      <FontPickerDialog open onClose={onClose} fonts={FONTS} value="Inter" onSelect={vi.fn()}
        trigger={<button type="button" aria-label="Fonts">Aa</button>} />,
...
describe("FontPickerDialog — anchored InspectorDialog contract", () => {
  it("mounts as a 240px, elevation-400, left-anchored collision-safe dialog", () => {
    ...
    expect(content.props["aria-label"]).toBe("Fonts");
  ...
  it("is non-modal and renders a searchable font list", () => {
    const { renderer } = renderOpen("light");
    const content = radix(renderer.root, "content");
    expect(content.props["aria-modal"]).toBe(false);
    expect(content.findAll(node => node.props["aria-label"] === "Search fonts").length).toBeGreaterThanOrEqual(1);
    expect(content.findAll(node => node.children?.[0] === "Inter").length).toBeGreaterThanOrEqual(1);
    expect(content.findAll(node => node.children?.[0] === "Merriweather").length).toBeGreaterThanOrEqual(1);
    act(() => renderer.unmount());
  });
