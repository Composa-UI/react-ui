...
// Token-compliance report — makes the hardcoded-hex debt (Foundations) visible.
// Status is enforced by the annotation contract: a component may claim
// tokensOnly only if its source is verified hex-free.
function tokenCompliance(components) {
  const ok = c => c.enforce && c.enforce.tokensOnly === true;
  const rows = components.map(c => {
    const note = (c.tokens && c.tokens.note) ? c.tokens.note : "";
    const status = ok(c) ? `<span class="tc-ok">✓ token-only</span>`
      : note ? `<span class="tc-warn">⚠ hardcoded values</span>`
      : `<span class="tc-na">— not asserted</span>`;
    return `<tr><td><a href="#${slug(c.component)}">${esc(c.component)}</a></td>
      <td>${status}</td><td class="muted">${esc(note || (ok(c) ? "Source verified hex-free." : ""))}</td></tr>`;
  }).join("");
  const n = components.filter(ok).length;
  return `<section id="token-compliance"><h1>Token compliance</h1>
    <p class="lede">Which annotated components are fully token-bound. Enforced by the annotation contract: a component may claim <code>tokensOnly</code> only if its source carries no hardcoded hex, so this is verified, not asserted. <b>${n} of ${components.length}</b> are verified token-only; the rest name their hardcoded values — that is the hardcoded-hex debt to burn down.</p>
    <table class="kv tc"><tr><th>Component</th><th>Status</th><th>Note</th></tr>${rows}</table></section>`;
}
...
function componentSection(c) {
  ...
  const usage = `<p class="intent">${esc(c.intent)}</p>
    ...
    ${slots}${states}`;
  const tokensOnly = c.enforce && c.enforce.tokensOnly === true;
  ...
  const style = `<h3>Tokens</h3><p class="muted">${styleNote}</p>${kv(c.tokens || {})}`;
...
<style>
  ...
  *{box-sizing:border-box}body{margin:0;font-family:var(--sans);color:var(--ink);background:var(--bg);-webkit-font-smoothing:antialiased}
  ...
  table.tc td{padding:7px 12px 7px 0;border-bottom:1px solid var(--line);font-size:13px}table.tc td:last-child{font-size:12px}
  .tc-ok{color:#1a7f37;font-weight:600}.tc-warn{color:#9a6700;font-weight:600}.tc-na{color:var(--muted)}
</style></head><body>
<div class="app">
